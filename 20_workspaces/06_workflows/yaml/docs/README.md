<!-- Generated by workflows.yaml docs/api/reference; do not edit. -->
# YAML API Reference

This reference is generated by the `docs/api/reference` YAML workflow. It describes the
standard library, runnable examples, and the documentation workflow itself.

Regenerate or verify committed output from `20_workspaces/06_workflows/yaml/`:

```bash
python interpreter.py api-docs.yaml docs/api/reference --output_dir=docs --mode=write
python interpreter.py api-docs.yaml docs/api/reference --output_dir=docs --mode=check
```

See the [definition authoring schema](schema.md) for metadata and composition fields.

## Definitions

| Definition | ID | Source | Tags |
| --- | --- | --- | --- |
| [Abstract Base](definitions/abstract/base.md) | `abstract/base` | `stdlib/stdlib.yaml` | core, abstract |
| [Load Standard Library Modules](definitions/builtin/load-modules.md) | `builtin/load-modules` | `stdlib/stdlib.yaml` | core, modules |
| [Run Command](definitions/builtin/run-command.md) | `builtin/run-command` | `stdlib/stdlib.yaml` | shell |
| [Clean Workspace](definitions/builtin/clean-workspace.md) | `builtin/clean-workspace` | `stdlib/stdlib.yaml` | files, destructive |
| [Logger](definitions/builtin/logger.md) | `builtin/logger` | `stdlib/stdlib.yaml` | logging |
| [Write Text File](definitions/builtin/files/write-text.md) | `builtin/files/write-text` | `stdlib/stdlib.files.yaml` | files, templates |
| [Create Directory](definitions/builtin/files/create-directory.md) | `builtin/files/create-directory` | `stdlib/stdlib.files.yaml` | files |
| [Initialize Current Git Workspace](definitions/builtin/git/init-current-workspace.md) | `builtin/git/init-current-workspace` | `stdlib/stdlib.git.yaml` | git |
| [Write Python Gitignore](definitions/builtin/git/gitignore-python.md) | `builtin/git/gitignore-python` | `stdlib/stdlib.git.yaml` | git, files, templates |
| [Create GitHub Repository](definitions/builtin/git/create-github-repository.md) | `builtin/git/create-github-repository` | `stdlib/stdlib.git.yaml` | git, github |
| [Render Markdown Template](definitions/builtin/template/render-markdown.md) | `builtin/template/render-markdown` | `stdlib/stdlib.template.yaml` | templates, documentation |
| [Prune Generated Definition Pages](definitions/builtin/docs/prune-definition-pages.md) | `builtin/docs/prune-definition-pages` | `stdlib/stdlib.doc.yaml` | documentation, files |
| [Render API Reference](definitions/builtin/docs/render-reference.md) | `builtin/docs/render-reference` | `stdlib/stdlib.doc.yaml` | documentation, templates |
| [API Documentation Catalog Sources](definitions/docs/catalog/modules.md) | `docs/catalog/modules` | `api-docs.yaml` | documentation, modules |
| [YAML API Reference Build](definitions/docs/api/reference.md) | `docs/api/reference` | `api-docs.yaml` | documentation, workflow |
| [Example Catalog Modules](definitions/examples/catalog/modules.md) | `examples/catalog/modules` | `examples/examples.yaml` | examples, modules |
| [Git Information](definitions/mixin/git-info.md) | `mixin/git-info` | `examples/deploy.yaml` | mixin, git, deployment |
| [Timestamp](definitions/mixin/timestamp.md) | `mixin/timestamp` | `examples/deploy.yaml` | mixin, deployment |
| [Cloud Deployer](definitions/cloud/deployer.md) | `cloud/deployer` | `examples/deploy.yaml` | example, deployment, cloud |
| [Initialize Git Example](definitions/example/git/init.md) | `example/git/init` | `examples/git-current-workspace.yaml` | example, git |
| [GitHub Repository Example](definitions/example/git/github.md) | `example/git/github` | `examples/git-current-workspace.yaml` | example, git, github |
| [Gitignore Template Example](definitions/example/git/gitignore.md) | `example/git/gitignore` | `examples/gitignore-template.yaml` | example, git, templates |
| [Obsidian Daily Note Template](definitions/example/obsidian-template.md) | `example/obsidian-template` | `examples/obsidian-template.yaml` | example, obsidian, templates |


## Tags

### abstract

- [Abstract Base](definitions/abstract/base.md) (`abstract/base`)


### cloud

- [Cloud Deployer](definitions/cloud/deployer.md) (`cloud/deployer`)


### core

- [Abstract Base](definitions/abstract/base.md) (`abstract/base`)
- [Load Standard Library Modules](definitions/builtin/load-modules.md) (`builtin/load-modules`)


### deployment

- [Git Information](definitions/mixin/git-info.md) (`mixin/git-info`)
- [Timestamp](definitions/mixin/timestamp.md) (`mixin/timestamp`)
- [Cloud Deployer](definitions/cloud/deployer.md) (`cloud/deployer`)


### destructive

- [Clean Workspace](definitions/builtin/clean-workspace.md) (`builtin/clean-workspace`)


### documentation

- [Render Markdown Template](definitions/builtin/template/render-markdown.md) (`builtin/template/render-markdown`)
- [Prune Generated Definition Pages](definitions/builtin/docs/prune-definition-pages.md) (`builtin/docs/prune-definition-pages`)
- [Render API Reference](definitions/builtin/docs/render-reference.md) (`builtin/docs/render-reference`)
- [API Documentation Catalog Sources](definitions/docs/catalog/modules.md) (`docs/catalog/modules`)
- [YAML API Reference Build](definitions/docs/api/reference.md) (`docs/api/reference`)


### example

- [Cloud Deployer](definitions/cloud/deployer.md) (`cloud/deployer`)
- [Initialize Git Example](definitions/example/git/init.md) (`example/git/init`)
- [GitHub Repository Example](definitions/example/git/github.md) (`example/git/github`)
- [Gitignore Template Example](definitions/example/git/gitignore.md) (`example/git/gitignore`)
- [Obsidian Daily Note Template](definitions/example/obsidian-template.md) (`example/obsidian-template`)


### examples

- [Example Catalog Modules](definitions/examples/catalog/modules.md) (`examples/catalog/modules`)


### files

- [Clean Workspace](definitions/builtin/clean-workspace.md) (`builtin/clean-workspace`)
- [Write Text File](definitions/builtin/files/write-text.md) (`builtin/files/write-text`)
- [Create Directory](definitions/builtin/files/create-directory.md) (`builtin/files/create-directory`)
- [Write Python Gitignore](definitions/builtin/git/gitignore-python.md) (`builtin/git/gitignore-python`)
- [Prune Generated Definition Pages](definitions/builtin/docs/prune-definition-pages.md) (`builtin/docs/prune-definition-pages`)


### git

- [Initialize Current Git Workspace](definitions/builtin/git/init-current-workspace.md) (`builtin/git/init-current-workspace`)
- [Write Python Gitignore](definitions/builtin/git/gitignore-python.md) (`builtin/git/gitignore-python`)
- [Create GitHub Repository](definitions/builtin/git/create-github-repository.md) (`builtin/git/create-github-repository`)
- [Git Information](definitions/mixin/git-info.md) (`mixin/git-info`)
- [Initialize Git Example](definitions/example/git/init.md) (`example/git/init`)
- [GitHub Repository Example](definitions/example/git/github.md) (`example/git/github`)
- [Gitignore Template Example](definitions/example/git/gitignore.md) (`example/git/gitignore`)


### github

- [Create GitHub Repository](definitions/builtin/git/create-github-repository.md) (`builtin/git/create-github-repository`)
- [GitHub Repository Example](definitions/example/git/github.md) (`example/git/github`)


### logging

- [Logger](definitions/builtin/logger.md) (`builtin/logger`)


### mixin

- [Git Information](definitions/mixin/git-info.md) (`mixin/git-info`)
- [Timestamp](definitions/mixin/timestamp.md) (`mixin/timestamp`)


### modules

- [Load Standard Library Modules](definitions/builtin/load-modules.md) (`builtin/load-modules`)
- [API Documentation Catalog Sources](definitions/docs/catalog/modules.md) (`docs/catalog/modules`)
- [Example Catalog Modules](definitions/examples/catalog/modules.md) (`examples/catalog/modules`)


### obsidian

- [Obsidian Daily Note Template](definitions/example/obsidian-template.md) (`example/obsidian-template`)


### shell

- [Run Command](definitions/builtin/run-command.md) (`builtin/run-command`)


### templates

- [Write Text File](definitions/builtin/files/write-text.md) (`builtin/files/write-text`)
- [Write Python Gitignore](definitions/builtin/git/gitignore-python.md) (`builtin/git/gitignore-python`)
- [Render Markdown Template](definitions/builtin/template/render-markdown.md) (`builtin/template/render-markdown`)
- [Render API Reference](definitions/builtin/docs/render-reference.md) (`builtin/docs/render-reference`)
- [Gitignore Template Example](definitions/example/git/gitignore.md) (`example/git/gitignore`)
- [Obsidian Daily Note Template](definitions/example/obsidian-template.md) (`example/obsidian-template`)


### workflow

- [YAML API Reference Build](definitions/docs/api/reference.md) (`docs/api/reference`)


## Relationship Graph

Solid arrows identify `extends`; dashed arrows identify mixin use.

```mermaid
flowchart TD
def_abstract_base["Abstract Base"]
def_builtin_load_modules["Load Standard Library Modules"]
def_builtin_load_modules --> def_abstract_base
def_builtin_run_command["Run Command"]
def_builtin_run_command --> def_abstract_base
def_builtin_clean_workspace["Clean Workspace"]
def_builtin_clean_workspace --> def_abstract_base
def_builtin_logger["Logger"]
def_builtin_logger --> def_abstract_base
def_builtin_files_write_text["Write Text File"]
def_builtin_files_write_text --> def_abstract_base
def_builtin_files_create_directory["Create Directory"]
def_builtin_files_create_directory --> def_abstract_base
def_builtin_git_init_current_workspace["Initialize Current Git Workspace"]
def_builtin_git_init_current_workspace --> def_abstract_base
def_builtin_git_gitignore_python["Write Python Gitignore"]
def_builtin_git_gitignore_python --> def_builtin_files_write_text
def_builtin_git_create_github_repository["Create GitHub Repository"]
def_builtin_git_create_github_repository --> def_abstract_base
def_builtin_template_render_markdown["Render Markdown Template"]
def_builtin_template_render_markdown --> def_abstract_base
def_builtin_docs_prune_definition_pages["Prune Generated Definition Pages"]
def_builtin_docs_prune_definition_pages --> def_abstract_base
def_builtin_docs_render_reference["Render API Reference"]
def_builtin_docs_render_reference --> def_abstract_base
def_docs_catalog_modules["API Documentation Catalog Sources"]
def_docs_api_reference["YAML API Reference Build"]
def_docs_api_reference --> def_builtin_docs_render_reference
def_examples_catalog_modules["Example Catalog Modules"]
def_mixin_git_info["Git Information"]
def_mixin_timestamp["Timestamp"]
def_cloud_deployer["Cloud Deployer"]
def_cloud_deployer --> def_abstract_base
def_cloud_deployer -.-> def_mixin_timestamp
def_cloud_deployer -.-> def_mixin_git_info
def_example_git_init["Initialize Git Example"]
def_example_git_init -.-> def_builtin_git_init_current_workspace
def_example_git_init -.-> def_builtin_git_gitignore_python
def_example_git_github["GitHub Repository Example"]
def_example_git_github -.-> def_builtin_git_init_current_workspace
def_example_git_github -.-> def_builtin_git_gitignore_python
def_example_git_github -.-> def_builtin_git_create_github_repository
def_example_git_gitignore["Gitignore Template Example"]
def_example_git_gitignore --> def_builtin_git_gitignore_python
def_example_obsidian_template["Obsidian Daily Note Template"]
def_example_obsidian_template --> def_builtin_files_write_text
```

## Module Imports

```mermaid
flowchart LR
def_builtin_load_modules["builtin/load-modules"] --> def_builtin_load_modules_module_1["stdlib.files.yaml"]
def_builtin_load_modules["builtin/load-modules"] --> def_builtin_load_modules_module_2["stdlib.git.yaml"]
def_builtin_load_modules["builtin/load-modules"] --> def_builtin_load_modules_module_3["stdlib.template.yaml"]
def_builtin_load_modules["builtin/load-modules"] --> def_builtin_load_modules_module_4["stdlib.doc.yaml"]
def_docs_catalog_modules["docs/catalog/modules"] --> def_docs_catalog_modules_module_1["examples/examples.yaml"]
def_examples_catalog_modules["examples/catalog/modules"] --> def_examples_catalog_modules_module_1["deploy.yaml"]
def_examples_catalog_modules["examples/catalog/modules"] --> def_examples_catalog_modules_module_2["git-current-workspace.yaml"]
def_examples_catalog_modules["examples/catalog/modules"] --> def_examples_catalog_modules_module_3["gitignore-template.yaml"]
def_examples_catalog_modules["examples/catalog/modules"] --> def_examples_catalog_modules_module_4["obsidian-template.yaml"]
```
