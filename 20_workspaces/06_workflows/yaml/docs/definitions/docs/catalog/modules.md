<!-- Generated by workflows.yaml docs/api/reference; do not edit. -->
# API Documentation Catalog Sources

[API index](../../../README.md) | [Definition schema](../../../schema.md)

| Field | Value |
| --- | --- |
| ID | `docs/catalog/modules` |
| Source | `api-docs.yaml` |
| Tags | documentation, modules |

Imports runnable notebooks and examples into the generated YAML API catalog.


## Relationships

| Relation | Definitions |
| --- | --- |
| Extends | - |
| Mixins | - |
| Children | - |
| Mixin consumers | - |
| Modules | `examples/deploy.yaml`, `notebooks/git-current-workspace.yaml`, `notebooks/gitignore-template.yaml`, `notebooks/obsidian-template.yaml` |

```mermaid
flowchart TD
  def_docs_catalog_modules["API Documentation Catalog Sources"]
```

## Declared API

### Inputs

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| - | - | - | - | No locally declared inputs. |


### Variables

| Name | YAML Type | Value / Template | Description |
| --- | --- | --- | --- |
| - | - | - | No locally declared variables. |


### Run Operations

_No locally declared run operations._

### Teardown Operations

_No locally declared teardown operations._

## Resolved API

### Effective Inputs

| Name | Type | Required | Default | Origin |
| --- | --- | --- | --- | --- |
| - | - | - | - | No effective inputs. |


### Effective Variables

| Name | Value / Template | Origin |
| --- | --- | --- |
| - | - | No effective variables. |


### Effective Lifecycle

| Phase | Operation | Origin |
| --- | --- | --- |
| - | - | No effective lifecycle operations. |
