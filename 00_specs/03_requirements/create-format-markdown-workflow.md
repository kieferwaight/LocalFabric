...
prompt_path: 12_prompts/spec.promotion.requirements.task.md
...
The notebook must load `12_prompts/text.markdown.format.task.md` using the standardized prompt registry/loader from `02_core`, treating it as an immutable input artifact without inline templating or fallback logic. Resolves research Q1.
...
**N5: Path Discipline**  
All file references must use relative paths anchored to the repository root, and the notebook must validate that `12_prompts/text.markdown.format.task.md` exists and is readable before execution begins.
