"""Smoke tests for the markdown CLI adapter (adapters.cli.markdown)."""
...
import pytest
from adapters.cli.markdown import main
...
def test_cli_runs_hello_shell_example() -> None:
    rc = main([str(EXAMPLES / "hello-shell.md")])
    assert rc == 0
...
def test_cli_runs_polyglot_example_with_debug(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main(
        [
            str(EXAMPLES / "hello-polyglot.md"),
            "--name=World",
            "--debug",
        ]
    )
