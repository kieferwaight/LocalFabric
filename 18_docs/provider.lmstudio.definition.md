...
| Source | `13_providers/provider.lmstudio.yaml` |
| Tags | provider |
...
def_provider_lmstudio --> def_provider_base["LLM Provider Record"]
```
...
| `name` | `lmstudio` | [lmstudio](provider.lmstudio.definition.md) |
| `location` | `local` | [lmstudio](provider.lmstudio.definition.md) |
| `documentation` | `{'summary': 'Desktop LLM runtime. Loads quantized open-weight models locally and serves\nthem through a native HTTP API plus an OpenAI-compatible surface.\n', 'homepage': 'https://lmstudio.ai'}` | [lmstudio](provider.lmstudio.definition.md) |
| `endpoint` | `http://localhost:1234` | [lmstudio](provider.lmstudio.definition.md) |
| `auth` | `none` | [lmstudio](provider.lmstudio.definition.md) |
| `env` | `{'host': 'LMSTUDIO_HOST', 'api_key': 'LMSTUDIO_API_KEY'}` | [lmstudio](provider.lmstudio.definition.md) |
| `protocols` | `{'cli': {'binary': 'lms'}, 'lmstudio-api': {'base_url': 'http://localhost:1234'}, 'openai-api': {'base_url': 'http://localhost:1234/v1'}}` | [lmstudio](provider.lmstudio.definition.md) |
