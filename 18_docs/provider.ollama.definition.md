...
| `name` | `ollama` | [ollama](provider.ollama.definition.md) |
| `location` | `local` | [ollama](provider.ollama.definition.md) |
| `documentation` | `{'summary': 'Local-first runtime for open-weight LLMs. Pulls model weights on demand and\nserves them through a daemon that ships both a native API and an OpenAI\nChat Completions-compatible surface.\n', 'homepage': 'https://ollama.com', 'install': 'brew install ollama'}` | [ollama](provider.ollama.definition.md) |
| `endpoint` | `http://localhost:11434` | [ollama](provider.ollama.definition.md) |
| `auth` | `none` | [ollama](provider.ollama.definition.md) |
| `env` | `{'host': 'OLLAMA_HOST'}` | [ollama](provider.ollama.definition.md) |
| `protocols` | `{'cli': {'binary': 'ollama', 'invocation': 'ollama run <model>'}, 'ollama-api': {'base_url': 'http://localhost:11434/api'}, 'openai-api': {'base_url': 'http://localhost:11434/v1'}}` | [ollama](provider.ollama.definition.md) |
