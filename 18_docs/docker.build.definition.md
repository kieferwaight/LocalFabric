...
via inputs). Wraps `stdlib.run-command`.
...
def_docker_build --> def_stdlib_run_command["Run Command"]
```
...
| `image_tag` | `string` | yes | `-` | [Build Docker Image](docker.build.definition.md) |
| `build_context` | `string` | yes | `-` | [Build Docker Image](docker.build.definition.md) |
...
| `command` | `docker build -t {{ image_tag }} {{ build_context }}` | [Build Docker Image](docker.build.definition.md) |
