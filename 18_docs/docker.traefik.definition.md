...
def_docker_traefik --> def_docker_base["Docker Service (Compose + Dockerfile)"]
```
...
| `TRAEFIK_HTTP_PORT` | `string` | no | `80` | [Traefik Service](docker.traefik.definition.md) |
| `TRAEFIK_HTTPS_PORT` | `string` | no | `443` | [Traefik Service](docker.traefik.definition.md) |
| `TRAEFIK_DASHBOARD_PORT` | `string` | no | `8080` | [Traefik Service](docker.traefik.definition.md) |
...
| `services` | `{'traefik': {'image': 'traefik:v3', 'container_name': 'contentgraph-traefik', 'restart': 'unless-stopped', 'command': ['--api.dashboard=true', '--api.insecure=true', '--providers.docker=true', '--providers.docker.exposedbydefault=false', '--entrypoints.web.address=:80', '--entrypoints.websecure.address=:443'], 'ports': ['{{ TRAEFIK_HTTP_PORT }}:80', '{{ TRAEFIK_HTTPS_PORT }}:443', '{{ TRAEFIK_DASHBOARD_PORT }}:8080'], 'volumes': ['/var/run/docker.sock:/var/run/docker.sock:ro', './config:/etc/traefik:ro']}}` | [Traefik Service](docker.traefik.definition.md) |
...
| `extra_files` | `[{'path': 'config/.gitkeep', 'content': ''}]` | [Traefik Service](docker.traefik.definition.md) |
