...
| --- | --- |
| ID | `provider.openai` |
| Source | `13_providers/provider.openai.yaml` |
| Tags | provider |
...
def_provider_openai --> def_provider_base["LLM Provider Record"]
```
...
| `name` | `openai` | [openai](provider.openai.definition.md) |
| `location` | `cloud` | [openai](provider.openai.definition.md) |
| `documentation` | `{'summary': "OpenAI's hosted model catalog (GPT-class chat, code, embedding, and vision).\nThe canonical owner of the OpenAI Chat Completions wire format.\n", 'homepage': 'https://platform.openai.com', 'vendor': 'OpenAI'}` | [openai](provider.openai.definition.md) |
| `endpoint` | `https://api.openai.com` | [openai](provider.openai.definition.md) |
| `auth` | `api_key` | [openai](provider.openai.definition.md) |
| `env` | `{'api_key': 'OPENAI_API_KEY'}` | [openai](provider.openai.definition.md) |
| `protocols` | `{'cli': {'binary': 'codex'}, 'sdk': {'package': 'openai', 'languages': ['python', 'typescript']}, 'openai-api': {'base_url': 'https://api.openai.com/v1'}}` | [openai](provider.openai.definition.md) |
