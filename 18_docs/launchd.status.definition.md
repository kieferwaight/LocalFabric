...
def_launchd_status --> def_launchd_lifecycle_base["launchd Lifecycle Base"]
```
...
| --- | --- | --- | --- |
| `command` | `str` | `launchctl print "gui/$UID/{{ resolved_label }}" 2>/dev/null \   \|\| echo "launchd job '{{ resolved_label }}' is not loaded (install with launchd.install)" ` | - |
...
| `command` | `launchctl print "gui/$UID/{{ resolved_label }}" 2>/dev/null \   \|\| echo "launchd job '{{ resolved_label }}' is not loaded (install with launchd.install)" ` | [Print LaunchAgent Status](launchd.status.definition.md) |
