...
across reboots and bootstraps until `launchd.enable.task` is invoked.
...
def_launchd_disable_task --> def_launchd_lifecycle_base["launchd Lifecycle Base"]
```
...
| `command` | `launchctl disable gui/$UID/{{ resolved_label }}` | [Disable LaunchAgent](launchd.disable.task.definition.md) |
