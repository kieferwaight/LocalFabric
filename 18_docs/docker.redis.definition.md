...
def_docker_redis --> def_docker_base["Docker Service (Compose + Dockerfile)"]
```
...
| `DATA_PATH` | `string` | yes | `-` | [Redis Service](docker.redis.definition.md) |
| `REDIS_PORT` | `string` | no | `6379` | [Redis Service](docker.redis.definition.md) |
...
| `services` | `{'redis': {'image': 'redis:7', 'container_name': 'contentgraph-redis', 'restart': 'unless-stopped', 'command': ['redis-server', '--appendonly', 'yes'], 'ports': ['{{ REDIS_PORT }}:6379'], 'volumes': ['{{ DATA_PATH }}:/data']}}` | [Redis Service](docker.redis.definition.md) |
