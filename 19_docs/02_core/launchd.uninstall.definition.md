<!-- Generated by docs.api.reference; do not edit. -->
# Uninstall LaunchAgent

[API index](index.md) | [Definition schema](schema.md)

| Field | Value |
| --- | --- |
| ID | `launchd.uninstall` |
| Source | `02_core/runtimes/yaml/definitions/launchd.yaml` |
| Tags | launchd, lifecycle |

Boots the job out of the user GUI domain and removes its `.plist` from
`~/Library/LaunchAgents/`. Safe to call against a job that is not
currently loaded.


## Relationships

| Relation | Definitions |
| --- | --- |
| Extends | [launchd Lifecycle Base](launchd.lifecycle.base.definition.md) |
| Mixins | - |
| Children | - |
| Mixin consumers | - |
| Modules | - |

```mermaid
flowchart LR
  def_launchd_uninstall["Uninstall LaunchAgent"]
def_launchd_uninstall --> def_launchd_lifecycle_base["launchd Lifecycle Base"]
```


## Declared API

### Inputs

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| - | - | - | - | No locally declared inputs. |


### Variables

| Name | YAML Type | Value / Template | Description |
| --- | --- | --- | --- |
| `command` | `str` | `launchctl bootout "gui/$UID/{{ resolved_label }}" 2>/dev/null \|\| true rm -f "$HOME/Library/LaunchAgents/{{ resolved_label }}.plist" ` | - |


### Run Operations

_No locally declared run operations._

### Teardown Operations

_No locally declared teardown operations._

## Resolved API

### Effective Inputs

| Name | Type | Required | Default | Origin |
| --- | --- | --- | --- | --- |
| `command` | `string` | no | `` | [Run Command](stdlib.run-command.definition.md) |
| `working_dir` | `string` | no | `{{ env.cwd }}` | [Run Command](stdlib.run-command.definition.md) |
| `definition` | `string` | no | `` | [launchd Lifecycle Base](launchd.lifecycle.base.definition.md) |
| `label` | `string` | no | `` | [launchd Lifecycle Base](launchd.lifecycle.base.definition.md) |


### Effective Variables

| Name | Value / Template | Origin |
| --- | --- | --- |
| `system_log_format` | `[{{ entity_id }} \| {{ runtime.version }}]` | [Abstract Base](stdlib.base.definition.md) |
| `resolved_label` | `{{ ((runtime.catalog.definitions \| selectattr('id', 'equalto', definition) \| list \| first).launchd.label) if definition else label }}` | [launchd Lifecycle Base](launchd.lifecycle.base.definition.md) |
| `command` | `launchctl bootout "gui/$UID/{{ resolved_label }}" 2>/dev/null \|\| true rm -f "$HOME/Library/LaunchAgents/{{ resolved_label }}.plist" ` | [Uninstall LaunchAgent](launchd.uninstall.definition.md) |


### Effective Lifecycle

| Phase | Operation | Origin |
| --- | --- | --- |
| Run | `bash` | [Run Command](stdlib.run-command.definition.md) |
