...
| `plist_path` | `string` | no | `` | [Install LaunchAgent](launchd.install.md) |
...
| `resolved_plist_path` | `{{ plist_path if plist_path else '09_launchd/' ~ definition ~ '.plist' }}` | [Install LaunchAgent](launchd.install.md) |
| `command` | `mkdir -p "$HOME/Library/LaunchAgents" cp "{{ resolved_plist_path }}" "$HOME/Library/LaunchAgents/{{ resolved_label }}.plist" launchctl bootout "gui/$UID/{{ resolved_label }}" 2>/dev/null \|\| true launchctl bootstrap "gui/$UID" "$HOME/Library/LaunchAgents/{{ resolved_label }}.plist" ` | [Install LaunchAgent](launchd.install.md) |
