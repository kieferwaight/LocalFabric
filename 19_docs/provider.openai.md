...
| `name` | `openai` | [openai](provider.openai.md) |
| `location` | `cloud` | [openai](provider.openai.md) |
| `documentation` | `{'summary': "OpenAI's hosted model catalog (GPT-class chat, code, embedding, and vision).\nThe canonical owner of the OpenAI Chat Completions wire format.\n", 'homepage': 'https://platform.openai.com', 'vendor': 'OpenAI'}` | [openai](provider.openai.md) |
| `endpoint` | `https://api.openai.com` | [openai](provider.openai.md) |
| `auth` | `api_key` | [openai](provider.openai.md) |
| `env` | `{'api_key': 'OPENAI_API_KEY'}` | [openai](provider.openai.md) |
| `protocols` | `{'cli': {'binary': 'codex'}, 'sdk': {'package': 'openai', 'languages': ['python', 'typescript']}, 'openai-api': {'base_url': 'https://api.openai.com/v1'}}` | [openai](provider.openai.md) |
