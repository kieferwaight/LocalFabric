...
| `DATABASE_URL` | `string` | yes | `-` | [FastAPI Service](docker.fastapi.md) |
| `NEO4J_PASSWORD` | `string` | yes | `-` | [FastAPI Service](docker.fastapi.md) |
| `FASTAPI_PORT` | `string` | no | `8000` | [FastAPI Service](docker.fastapi.md) |
| `NEO4J_URI` | `string` | no | `bolt://neo4j:7687` | [FastAPI Service](docker.fastapi.md) |
| `NEO4J_USER` | `string` | no | `neo4j` | [FastAPI Service](docker.fastapi.md) |
| `REDIS_URL` | `string` | no | `redis://redis:6379/0` | [FastAPI Service](docker.fastapi.md) |
| `MINIO_ENDPOINT` | `string` | no | `minio:9000` | [FastAPI Service](docker.fastapi.md) |
| `TIKA_URL` | `string` | no | `http://tika:9998` | [FastAPI Service](docker.fastapi.md) |
...
| `dockerfile_body` | `FROM python:3.12-slim  WORKDIR /app RUN pip install --no-cache-dir fastapi uvicorn[standard] psycopg[binary] redis neo4j boto3 httpx COPY app /app  EXPOSE 8000 CMD ["uvicorn", "main:app", "--host", "0.0.0.0", "--port", "8000"] ` | [FastAPI Service](docker.fastapi.md) |
| `image_tag` | `contentgraph-api:local` | [FastAPI Service](docker.fastapi.md) |
| `build_context` | `{{ target_dir }}` | [Dockerfile Mixin](dockerfile.base.md) |
| `services` | `{'contentgraph-api': {'build': '.', 'image': 'contentgraph-api:local', 'container_name': 'contentgraph-api', 'restart': 'unless-stopped', 'environment': {'DATABASE_URL': '{{ DATABASE_URL }}', 'NEO4J_URI': '{{ NEO4J_URI }}', 'NEO4J_USER': '{{ NEO4J_USER }}', 'NEO4J_PASSWORD': '{{ NEO4J_PASSWORD }}', 'REDIS_URL': '{{ REDIS_URL }}', 'MINIO_ENDPOINT': '{{ MINIO_ENDPOINT }}', 'TIKA_URL': '{{ TIKA_URL }}'}, 'ports': ['{{ FASTAPI_PORT }}:8000']}}` | [FastAPI Service](docker.fastapi.md) |
