...
| --- | --- |
| ID | `provider.claude` |
| Source | `13_providers/provider.claude.yaml` |
| Tags | provider |
...
def_provider_claude --> def_provider_base["LLM Provider Record"]
```
...
| `name` | `claude` | [claude](provider.claude.definition.md) |
| `location` | `cloud` | [claude](provider.claude.definition.md) |
| `documentation` | `{'summary': "Anthropic's hosted Claude models. Accessed via the Anthropic Messages API\n(no OpenAI-compatible endpoint), the official Anthropic SDKs, or the Claude\nCode CLI.\n", 'homepage': 'https://www.anthropic.com', 'vendor': 'Anthropic'}` | [claude](provider.claude.definition.md) |
| `endpoint` | `https://api.anthropic.com` | [claude](provider.claude.definition.md) |
| `auth` | `api_key` | [claude](provider.claude.definition.md) |
| `env` | `{'api_key': 'ANTHROPIC_API_KEY'}` | [claude](provider.claude.definition.md) |
| `protocols` | `{'cli': {'binary': 'claude'}, 'sdk': {'package': 'anthropic', 'languages': ['python', 'typescript']}, 'anthropic-api': {'base_url': 'https://api.anthropic.com'}}` | [claude](provider.claude.definition.md) |
