...
| `OPENFGA_HTTP_PORT` | `string` | no | `8083` | [OpenFGA Service](docker.openfga.definition.md) |
| `OPENFGA_GRPC_PORT` | `string` | no | `8081` | [OpenFGA Service](docker.openfga.definition.md) |
| `OPENFGA_PLAYGROUND_PORT` | `string` | no | `3003` | [OpenFGA Service](docker.openfga.definition.md) |
...
| `services` | `{'openfga': {'image': 'openfga/openfga', 'container_name': 'contentgraph-openfga', 'restart': 'unless-stopped', 'command': 'run --playground-enabled --datastore-engine memory', 'ports': ['{{ OPENFGA_HTTP_PORT }}:8080', '{{ OPENFGA_GRPC_PORT }}:8081', '{{ OPENFGA_PLAYGROUND_PORT }}:3000']}}` | [OpenFGA Service](docker.openfga.definition.md) |
