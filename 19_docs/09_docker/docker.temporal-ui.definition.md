...
| Source | `09_docker/temporal-ui.yaml` |
| Tags | docker, workers |
...
def_docker_temporal_ui --> def_docker_base["Docker Service (Compose + Dockerfile)"]
```
...
| `TEMPORAL_UI_PORT` | `string` | no | `8084` | [Temporal UI Service](docker.temporal-ui.definition.md) |
| `TEMPORAL_ADDRESS` | `string` | no | `temporal:7233` | [Temporal UI Service](docker.temporal-ui.definition.md) |
...
| `services` | `{'temporal-ui': {'image': 'temporalio/ui', 'container_name': 'contentgraph-temporal-ui', 'restart': 'unless-stopped', 'environment': {'TEMPORAL_ADDRESS': '{{ TEMPORAL_ADDRESS }}', 'TEMPORAL_CORS_ORIGINS': 'http://localhost:{{ TEMPORAL_UI_PORT }}'}, 'ports': ['{{ TEMPORAL_UI_PORT }}:8080']}}` | [Temporal UI Service](docker.temporal-ui.definition.md) |
