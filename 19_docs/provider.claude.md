...
| `name` | `claude` | [claude](provider.claude.md) |
| `location` | `cloud` | [claude](provider.claude.md) |
| `documentation` | `{'summary': "Anthropic's hosted Claude models. Accessed via the Anthropic Messages API\n(no OpenAI-compatible endpoint), the official Anthropic SDKs, or the Claude\nCode CLI.\n", 'homepage': 'https://www.anthropic.com', 'vendor': 'Anthropic'}` | [claude](provider.claude.md) |
| `endpoint` | `https://api.anthropic.com` | [claude](provider.claude.md) |
| `auth` | `api_key` | [claude](provider.claude.md) |
| `env` | `{'api_key': 'ANTHROPIC_API_KEY'}` | [claude](provider.claude.md) |
| `protocols` | `{'cli': {'binary': 'claude'}, 'sdk': {'package': 'anthropic', 'languages': ['python', 'typescript']}, 'anthropic-api': {'base_url': 'https://api.anthropic.com'}}` | [claude](provider.claude.md) |
