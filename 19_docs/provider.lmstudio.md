...
| `name` | `lmstudio` | [lmstudio](provider.lmstudio.md) |
| `location` | `local` | [lmstudio](provider.lmstudio.md) |
| `documentation` | `{'summary': 'Desktop LLM runtime. Loads quantized open-weight models locally and serves\nthem through a native HTTP API plus an OpenAI-compatible surface.\n', 'homepage': 'https://lmstudio.ai'}` | [lmstudio](provider.lmstudio.md) |
| `endpoint` | `http://localhost:1234` | [lmstudio](provider.lmstudio.md) |
| `auth` | `none` | [lmstudio](provider.lmstudio.md) |
| `env` | `{'host': 'LMSTUDIO_HOST', 'api_key': 'LMSTUDIO_API_KEY'}` | [lmstudio](provider.lmstudio.md) |
| `protocols` | `{'cli': {'binary': 'lms'}, 'lmstudio-api': {'base_url': 'http://localhost:1234'}, 'openai-api': {'base_url': 'http://localhost:1234/v1'}}` | [lmstudio](provider.lmstudio.md) |
