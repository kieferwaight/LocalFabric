<!-- Generated by docs.api.reference; do not edit. -->
# Build Docker Image

[API index](index.md) | [Definition schema](schema.md)

| Field | Value |
| --- | --- |
| ID | `docker.build` |
| Source | `02_core/runtimes/yaml/definitions/docker.yaml` |
| Tags | docker, build |

Builds the docker image for a `docker.base` child. Reads `image_tag`
and `build_context` from the child's variables (or accepts overrides
via inputs). Wraps `stdlib.run-command`.


## Relationships

| Relation | Definitions |
| --- | --- |
| Extends | [Run Command](stdlib.run-command.md) |
| Mixins | - |
| Children | - |
| Mixin consumers | - |
| Modules | - |

```mermaid
flowchart LR
  def_docker_build["Build Docker Image"]
def_docker_build --> def_stdlib_run_command["Run Command"]
```


## Declared API

### Inputs

| Name | Type | Required | Default | Description |
| --- | --- | --- | --- | --- |
| `image_tag` | `string` | yes | `-` | Image tag to build (e.g. `worker:local`). |
| `build_context` | `string` | yes | `-` | Directory passed to `docker build` as the context. |


### Variables

| Name | YAML Type | Value / Template | Description |
| --- | --- | --- | --- |
| `command` | `str` | `docker build -t {{ image_tag }} {{ build_context }}` | - |


### Run Operations

_No locally declared run operations._

### Teardown Operations

_No locally declared teardown operations._

## Resolved API

### Effective Inputs

| Name | Type | Required | Default | Origin |
| --- | --- | --- | --- | --- |
| `command` | `string` | no | `` | [Run Command](stdlib.run-command.md) |
| `working_dir` | `string` | no | `{{ env.cwd }}` | [Run Command](stdlib.run-command.md) |
| `image_tag` | `string` | yes | `-` | [Build Docker Image](docker.build.md) |
| `build_context` | `string` | yes | `-` | [Build Docker Image](docker.build.md) |


### Effective Variables

| Name | Value / Template | Origin |
| --- | --- | --- |
| `system_log_format` | `[{{ entity_id }} \| {{ runtime.version }}]` | [Abstract Base](stdlib.base.md) |
| `command` | `docker build -t {{ image_tag }} {{ build_context }}` | [Build Docker Image](docker.build.md) |


### Effective Lifecycle

| Phase | Operation | Origin |
| --- | --- | --- |
| Run | `bash` | [Run Command](stdlib.run-command.md) |
