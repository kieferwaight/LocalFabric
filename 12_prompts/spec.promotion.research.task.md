...
1. Has a section listing existing repo assets the idea must reuse or respect.
   Reference files by relative path when you can infer them from the idea.
1. Has a section listing layer-ownership constraints if the idea touches
   buckets like `02_core`, `03_adapters`, `04_harnesses`, `05_router`,
   `06_workflows`, `08_drivers`, `11_mcp`, or `12_prompts`.
1. Has a section listing external dependencies (services, models, libraries).
1. Ends with a numbered list of open questions for the requirements stage.
